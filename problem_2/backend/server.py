...
sys.path.insert(0, r'D:\machine-intelligence\problem_2\test')
...
def solve_puzzle():
    try:
        ...
        if algorithm == "pattern_db" and goal != standard_goal:
            print(f"[INFO] 目标状态非标准（模式数据库仅支持标准目标），切换到线性冲突算法")
            algorithm = "linear_conflict"
        ...
        if algorithm == "pattern_db" and size == 4:
            ...
            if len(pattern_dbs) == 3:
                try:
                    ...
                    forward_path, backward_path = main2.solve_with_custom_goal(start, goal, size, pattern_dbs)
                    print(f"[DEBUG] 求解完成: forward_path长度={len(forward_path) if forward_path else 0}, backward_path长度={len(backward_path) if backward_path else 0}")
